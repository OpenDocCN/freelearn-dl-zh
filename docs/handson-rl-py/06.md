...
由于每台老虎机都通过自己的概率分布为我们提供奖励，因此我们的目标是找出哪台老虎机将在一段时间内为我们提供最大的累积奖励。 因此，智能体在每个时间步`t`上执行动作`aₜ`，即从投币游戏机中拉出一条手臂并获得奖励`Rₜ`<sub class="calibre24">和</sub>，我们智能体的目标是使累积奖励最大化。
