...
在前面的等式中，我们可以理解，最大化`Mᵢ`可以保证我们期望收益的最大化。 因此，现在我们的目标是最大化`Mᵢ`，从而最大化我们的预期回报。 由于我们使用参数化策略，因此在上一个公式中将`π`替换为`θ`，然后使用`θ_old`表示我们要改进的策略，如下所示：
...
`c₁`和`c₂`是系数，`L[t]^(VP)`是实际值和目标值函数之间的平方误差损失，即：
