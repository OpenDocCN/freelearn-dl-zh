...
我们在这里描述的 RNN 在某种程度上等同于一个单层的常规神经网络（并且具有额外的递归关系）。正如我们现在从第一章《神经网络的基本原理》中所知，单层网络存在一些严重的限制。别担心！像常规网络一样，我们可以堆叠多个 RNN，形成**堆叠 RNN**。在时间步`t`，位于第`l`层的 RNN 单元的细胞状态，`s^l[t]`，将接收来自第`l-1`层 RNN 单元的输出，`y^(l-1)[t]`，以及该层第`l`单元在时间步`t-1`的前一时刻的细胞状态，`s^l[t-1]`，作为输入：
