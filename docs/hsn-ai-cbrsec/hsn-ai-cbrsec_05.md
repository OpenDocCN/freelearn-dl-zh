...
将 *θ* 阈值移到方程的左侧，并将其与 `x₀` 变量关联（从而引入求和的 *i = 0* 位置索引），并为 `x₀` 分配常规值 1，同时将权重 `w₀` 设为 *-θ*（即阈值的负值，跟随 *θ* 向方程左侧的位移）。因此，用 *θ* 替代乘积：
...
我们看到，它是 ![](img/ab8d4619-eaa0-4f07-aaa6-634ba22ef1e2.png) 值的乘积（即对应权重的输入数据），必须超过 *θ* 阈值才能确定感知机是否激活。由于 `xᵢ` 输入数据按定义是预先设定的，正是对应权重的值帮助判断感知机是否需要激活。
