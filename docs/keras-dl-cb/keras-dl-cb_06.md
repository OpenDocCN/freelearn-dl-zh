...
假设我们有一个监督学习任务，其中 `xᵢ` 是数据点的给定特征，`yᵢ` 是对应的标签。预测未来 `x` 上的 `y` 的一种方法是学习一个函数 *f()*，它从 *(x[i], y[i])* 中获取 `x` 并输出最可能的 `y`。这样的模型属于判别模型，因为你正在学习如何区分来自不同类别的 `x`。像 SVM 和神经网络这样的算法就属于这一类。即使你能够非常准确地分类数据，你也无法知道数据是如何生成的。
