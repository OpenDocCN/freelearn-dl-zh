...
本质上，这就是 `W` 的第 *k^(th)* 行。我们称之为 *ν[wIᐪ]*。从隐藏层到输出矩阵，有一个不同的权重 *W^' = {w[ij]^'}*，它是一个 `N` x `V` 矩阵。使用这些权重，你可以为词汇表中的每个单词计算一个分数 `u[j]`：
...
`u[j]` 和 *ν[w]^'* 是单词 `w` 的两种表示。 `u[j]` 来自 `W` 的行，`W` 是输入到隐藏权重矩阵的，而 *ν[w]^'* 来自 *W^'* 的列，*W^'* 是隐藏输出矩阵。在后续分析中，我们将称 `u[j]` 为单词 `w` 的“输入向量”，而 *ν[w]^'* 为其“输出向量”。考虑 `u[j]` 为描述的分数，我们可以将损失函数从方程 (6) 转换为如下形式：
