...
假设我们有一个由 ![](img/B15558_09_118.png) 参数化的模型 `f`，即 ![](img/B15558_17_022.png)，并且我们有一个任务分布 *p(T)*。首先，我们用一些随机值初始化我们的参数 ![](img/B15558_09_056.png)。接下来，我们从任务分布中抽取一批任务 `Tᵢ`―即 `Tᵢ`* ~ p(T)*。假设我们抽取了五个任务：`T[1]`、`T[2]`、`T[3]`、`T[4]`、`T[5]`。
...
接下来，我们从任务分布中采样一批任务 `Tᵢ`，即 `Tᵢ` *~ p(T)*。假设我们已经采样了三个任务，那么我们将得到 `T[1]`、`T[2]`、`T[3]`。
...
接下来，我们从任务分布中抽样一批任务`Tᵢ`，即`Tᵢ` *~ p(T)*。假设我们抽样了三个任务，那么我们有`T[1]`，`T[2]`，`T[3]`。
