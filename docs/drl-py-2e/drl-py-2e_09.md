...
这种高估是由于最大值操作符的存在。让我们通过一个例子来看看这种高估是如何发生的。假设我们处于状态 ![](img/B15558_09_100.png)，并且我们有三个动作 `a₁`、`a₂` 和 `a₃`。假设 `a₃` 是状态 ![](img/B15558_09_101.png) 下的最优动作。当我们估计状态 ![](img/B15558_03_004.png) 中所有动作的 Q 值时，估计的 Q 值会有一些噪声，并与实际值不同。比如，由于噪声，动作 `a₂` 的 Q 值会比最优动作 `a₃` 的 Q 值高。

我们知道目标值是这样计算的：

![](img/B15558_09_103.png)

现在，如果我们选择最大值对应的动作作为最佳动作，那么我们最终会选择动作 `a₂` 而不是最优动作 `a₃`，如图所示：
