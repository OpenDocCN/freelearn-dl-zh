...
在*图 2*中，我们有一些神经网络称为`A`，它接收输入 `Xₜ` 并生成输出 `hₜ`。同时，它借助这个循环接收来自过去步骤的信息。
...
我们的基础 LSTM 架构的第一步是决定哪些信息是不必要的，它通过丢弃这些信息，为更重要的信息留出更多空间。为此，我们有一个叫做**遗忘门层**的层，它查看前一个输出*h[t-1]*和当前输入`xₜ`，并决定我们要丢弃哪些信息。
