...
为了避免长时间的训练过程，可以选择每 `k₁` 个时间步计算一次梯度，而不是每一步计算一次。这将梯度计算的次数除以 `k₁`，使得网络的训练更加迅速。

我们可以将反向传播限制在 `k₂` 步之前，而不是遍历所有时间步。这有效地限制了梯度消失，因为梯度最多只会依赖于 *W^(k[2])*。这也限制了计算梯度所需的计算量。然而，网络将不太可能学习长期的时间关系。

这两种技术的结合被称为 **截断反向传播**，其两个参数通常被称为 `k₁` 和 `k₂`。它们必须进行调整，以确保在训练速度和模型性能之间达到良好的平衡。
