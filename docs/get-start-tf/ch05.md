...
现在让我们检查 RNN 的工作方式。`Xₜ`； 是时间`t`上的网络输入，例如可以是代表句子的单词，而`Sₜ`的向量。 是网络的*状态向量*。 可以认为是系统的一种*记忆*，其中包含有关输入序列的所有先前元素的信息。 从*当前输入*（时间`t`）开始求值`t`时刻的状态向量，并通过`U`和`W`参数的先前时刻（时间`t-1`）求值状态：

```py
S[t] = f([U] * X[t] + [W] * S[t-1])
```

函数`f`是*非线性函数*，例如整流线性单元（ReLu），而`Oₜ`； 是使用参数`V`计算的`t`时刻的输出。
...
LSTM 单元具有三个门和四个输入权重， `xₜ`（从数据到输入和三个门），而`hₜ`是单元的输出。
