...
*图 5.8* 显示了一个 CRBM（卷积限制玻尔兹曼机），其中包含一个可见单元矩阵`V`和一个隐藏单元矩阵`H`，它们通过*K 3`3`的滤波器连接，即*`W[1]`*，*`W[2]`*，*`W[3]`*，...*`W[K]`*。图中的隐藏单元被分成`K`个子矩阵，称为特征图，*`H[1]`*，*`H[2]`*，...*`H[K]`*。每个隐藏单元*`Hᵢ`*表示在可见单元的`3`3*邻域中某个特征的存在。
