...
现在让我们尝试编写代码，来学习模型的参数*θ*。但是，如我们所知，在计算机上寻找最优值时可能会遇到数值问题，是否有可能避免这些问题，直接计算`$1`呢？如果我们仔细观察我们的似然方程，我们会发现我们可以为这个模型写出一个通用的似然公式。如果我们假设我们的数据中有`n`个正面和`m`个反面，我们可以将似然写成如下形式：

![](img/1c241c3f-451d-4b92-9ec5-a9a6576f5673.png)

现在，我们实际上可以使用这个似然函数找到`$1`的封闭形式解，避免依赖任何数值方法来计算最优值：
...
我们可以通过对每个变量分别求偏导数，令其等于`0`，并解方程来找到最大化对数似然函数的`$1`和`$1`的值。为了得到均值，我们需要对对数似然函数关于*μ*求偏导数，同时保持*σ*为常数，并设其为`0`，得到以下结果：
...
正如我们在前几章中讨论的，潜在变量`zₙ`可以表示为`K`维的二元变量，当`zₙ`处于状态`k`时，*z[nk] = 1*。我们还可以用它表示*z[nk] = 1*的条件概率，同样，*ξ(z[n-1], j, z[nk])*可以表示*z[n-1]*、*j = 1*和*z[nk] = 1*的条件概率。由于二元随机变量的期望就是其值为`1`的概率，我们可以得出以下结论：
...
如果我们假设发射分布是正态分布，令 ![](img/dcabab1e-ffd3-473c-a5e6-8d5c9e8f1fd3.png)，那么对*Q(θ, θ^(old))* 关于`$1`的最大化将得到如下结果：
