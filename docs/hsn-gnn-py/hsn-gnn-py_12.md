...
`Nᵢ`是`i`节点的邻居节点集合（包括自身），`hᵢ`是`i`节点的嵌入，`W`是一个权重矩阵：
...
+   **Update**：每个节点使用一个函数更新其特征，将其当前特征与聚合的消息结合。在前面的例子中，我们引入了自环来聚合`i`节点的当前特征，就像一个邻居一样。

这些步骤可以总结为一个公式：

![](img/Formula_B19153_12_008.jpg)

这里，`hᵢ`是`i` 节点的嵌入`e[j,i]` 是`j -> i`链接的边嵌入，`Φ`是消息函数，`⊕`是聚合函数，`γ`是更新函数。你可以在下图中找到这个框架的示意图：
...
异构图是一种更通用的网络，可以表示不同类型的节点和边。形式上，它被定义为一个图，`G = (V, E)`，包含`V`，一组节点，和`E`，一组边。在异构设置中，它与一个节点类型映射函数`Φ: V -> A`相关联（其中`A`表示节点类型的集合），以及一个链接类型映射函数`Ψ: E -> R`（其中`R`表示边类型的集合）。
...
类似于 GAT，第一步是将节点映射到每个元路径的统一特征空间。接着，我们使用第二个权重矩阵计算在同一元路径中节点对（两个投影节点的连接）的权重。然后对该结果应用非线性函数，并通过 softmax 函数进行归一化。归一化后的注意力分数（重要性）表示为从`j`节点到`i`节点的计算方式如下：

![](img/Formula_B19153_12_027.jpg)

在这里，`hᵢ`表示`i`节点的特征，`W[Φ]`是`Φ`元路径的共享权重矩阵，`a[Φ]`是`Φ`元路径的注意力权重矩阵，`σ`是非线性激活函数（如 LeakyReLU），而`N[i]^σ`是`i`节点在`Φ`元路径中的邻居集合（包括其自身）。

也会执行多头注意力机制以获得最终的嵌入表示：

![](img/Formula_B19153_12_038.jpg)

通过语义级别注意力，我们对每个元路径的注意力分数（记作`β[Φ1], β[Φ2], ..., β[ΦP]`）执行类似的处理。每个给定元路径中的节点嵌入（记作`Z[ΦP]`）输入到 MLP 中，MLP 应用非线性变换。我们将此结果与一个新的注意力向量`q`进行比较，作为相似度度量。然后我们对该结果求平均，以计算给定元路径的重要性：

![](img/Formula_B19153_12_042.jpg)

在这里，`W`（MLP 的权重矩阵）、`b`（MLP 的偏置）和`q`（语义级别的注意力向量）在所有元路径之间共享。

我们必须对这个结果进行归一化，以比较不同的语义级注意力分数。我们使用 softmax 函数来获取我们的最终权重：

![](img/Formula_B19153_12_046.jpg)

结合节点级和语义级注意力的最终嵌入，如下所示：`Z`
