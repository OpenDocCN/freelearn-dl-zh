...
基本的神经网络层对应于线性变换`hₐ = xₐWᵀ`，其中`xₐ`是节点`A`的输入向量，`W`是权重矩阵。在 PyTorch 中，这个方程可以通过 `torch.mm()` 函数实现，或者使用 `nn.Linear` 类，它会添加其他参数，比如偏置项。
...
让我们称`Nₐ`为节点`A`的邻居集。我们的 **图线性层** 可以如下书写：

![](img/Formula_B19153_05_006.jpg)

你可以想象这个方程的几种变体。例如，我们可以为中央节点设置一个权重矩阵`W₁`，为邻居节点设置另一个权重矩阵`W₂`。需要注意的是，我们不能为每个邻居节点设置一个权重矩阵，因为邻居的数量会根据节点不同而变化。

我们讨论的是神经网络，所以我们不能对每个节点应用之前的方程。相反，我们执行矩阵乘法，这种方式更高效。例如，线性层的方程可以重写为 `H = XWᵀ`，其中`X`是输入矩阵。

在我们的例子中，邻接矩阵`A`包含了图中每个节点之间的连接。将输入矩阵与这个邻接矩阵相乘，将直接累加邻居节点的特征。我们可以将 `self` 循环添加到邻接矩阵中，这样就可以在这个操作中考虑中央节点。我们称这个更新后的邻接矩阵为`A_tilde = A + 1`。我们的图线性层可以重新编写如下：
