...
1.  我们创建一个函数来计算邻接矩阵，该函数需要三个参数：距离矩阵和两个阈值`σ²`和`ε`。像官方实现一样，我们将距离除以 10,000 并计算`d²`：
...
1.  在这里，我们希望在权重值大于或等于`ε`时保留这些权重（否则它们应等于零）。当我们测试权重是否大于或等于`ε`时，结果是`True`或`False`语句。这就是为什么我们需要一个全是 1 的掩码（`w_mask`）来将其转换回 0 和 1 值的原因。我们第二次乘以它，以便只获得大于或等于`ε`的真实权重值：
