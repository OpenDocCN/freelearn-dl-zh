...
GAT 的主要思想是，有些节点比其他节点更重要。实际上，这一点在图卷积层中就已经存在：具有较少邻居的节点比其他节点更重要，因为它们有归一化系数`1/(√deg(i)√deg(j))`。这种方法存在局限性，因为它只考虑节点的度数。另一方面，图注意力层的目标是生成考虑节点特征重要性的加权因子。

我们将我们的加权因子称为**注意力得分**，并注意到，`α[ij]`，表示节点`i`和`j`之间的注意力得分。我们可以将图注意力算子定义如下：
...
注意力得分表示一个中心节点`i`与一个邻居节点`j`之间的重要性。如前所述，这需要来自两个节点的节点特征。在图注意力层中，它通过隐藏向量`Wxᵢ`和`Wxⱼ`的连接来表示，`[Wxᵢ || Wxⱼ]`。在这里，`W`是一个经典的共享权重矩阵，用于计算隐藏向量。一个额外的线性变换被应用于这个结果，并使用一个专门的可学习权重矩阵 `W_att`。在训练过程中，这个矩阵学习权重以生成注意力系数 `a[ij]`。这个过程可以通过以下公式总结：
...
我们希望比较不同的注意力得分，这意味着我们需要在同一尺度上进行标准化的值。在机器学习中，通常使用 softmax 函数来实现这一点。我们称`Nᵢ` 为节点`i` 的邻居节点，包括它本身：

![](img/Formula_B19153_07_018.jpg)

该操作的结果给出了我们的最终注意力得分`α[ij]`。但是还有另一个问题：自注意力并不是非常稳定。
...
+   **平均**：在这种方法中，我们对不同的嵌入进行求和，并通过注意力头的数量`n`对结果进行归一化：
...
他们的解决方案是修改操作的顺序。权重矩阵`w`在拼接之后应用，注意力权重矩阵`W_att`在`LReLU`函数之后应用。总结一下，下面是原始的 **图注意力操作符**，即 **GAT**：
...
图必须提供两项重要信息：带有自环的邻接矩阵`A_tilde`和节点特征`X`。让我们看看如何在 NumPy 中实现它：
...
1.  对于`X`，我们使用`np.random.uniform()`生成一个随机的节点特征矩阵：
...
1.  下一步是定义我们的权重矩阵。实际上，在图注意力层中，有两个权重矩阵：常规权重矩阵`W`和注意力权重矩阵`W_att`。初始化它们的方式有很多种（例如 Xavier 或 He 初始化），但在这个例子中，我们可以简单地重用相同的随机函数。

矩阵`W`必须精心设计，因为它的维度是`(nb_og_hidden_dimensions, nb_of_nodes)`。注意，`nb_of_nodes = 4` 已经是固定的，因为它表示`X`中的节点数。相反，`nb_og_hidden_dimensions` 的值是任意的：我们将在这个例子中选择`2`：
...
1.  这个注意力矩阵应用于隐藏向量的拼接，生成一个唯一的值。因此，它的大小需要是`(1. dim_h × 2)`：
...
1.  我们希望将源节点和目标节点的隐藏向量进行拼接。获取源节点和目标节点对的一种简单方法是查看我们的邻接矩阵`A_tilde`，它采用 COO 格式：行存储源节点，列存储目标节点。NumPy 提供了一种快速高效的方法，使用`np.where()`来实现：
...
1.  然后我们使用注意力矩阵`W_att` 对该结果进行线性变换：
...
1.  我们有正确的值，但需要将它们正确地放置在矩阵中。这个矩阵应该看起来像`A_tilde`，因为当两个节点之间没有连接时，不需要标准化的注意力分数。为了构建这个矩阵，我们通过`connections`知道源节点`i`和目标节点`j`。因此，`e`中的第一个值对应于`e₀₀`，第二个值对应于`e₀₁`，但第七个值对应于`e₂₀`，而不是 `e₁₂`。我们可以按如下方式填充矩阵：
...
1.  这个注意力矩阵`W[α]`为网络中每一个可能的连接提供了权重。我们可以用它来计算我们的嵌入矩阵 `H`，该矩阵应为每个节点提供二维向量：
...
我们的图注意力层现在已经完成！添加多头注意力的过程包括在聚合结果之前使用不同的`W`和`W_att` 重复这些步骤。
