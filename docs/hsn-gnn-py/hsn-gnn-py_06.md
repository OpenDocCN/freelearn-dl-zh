...
首先，让我们讨论一个我们在上一章没有预见到的问题。与表格数据或图像数据不同，节点的邻居数并不总是相同。例如，在*图 6.1*中，节点`1`有 3 个邻居，而节点`2`只有 1 个：

![图 6.1 – 简单图，其中节点的邻居数不同](img/B19153_06_001.jpg)

图 6.1 – 简单图，其中节点的邻居数不同

然而，如果我们观察我们的 GNN 层，我们并没有考虑邻居数量的差异。我们的层由一个简单的求和构成，没有任何归一化系数。下面是我们计算节点嵌入的方式，`i`：

![](img/Formula_B19153_06_004.jpg)

假设节点`1`有 1000 个邻居，而节点`2`只有 1 个：嵌入`h[A]`的值将比`h[B]`大得多。这是一个问题，因为我们想要比较这些嵌入。当它们的值差异如此之大时，我们该如何进行有意义的比较呢？

幸运的是，有一个简单的解决方法：通过邻居数量来除以嵌入。让我们写出 `deg(A)`，节点`A`的度。这里是 GNN 层的新公式：
...
这个公式中唯一缺少的是一个矩阵，用来给我们提供归一化系数，`1/deg(i)`。这一点可以通过度矩阵`D`获得，它计算每个节点的邻居数量。下面是图 6.1 中所示图形的度矩阵：
...
根据定义，`D`给出了每个节点的度，`deg(i)`。因此，这个矩阵的逆 `D⁻¹`直接给我们归一化系数，`1/deg(i)`：
...
这正是我们所寻找的。为了更加准确，我们在图中添加了自环，用`A_tilde = A + 1`表示。同样，我们应该在度矩阵中添加自环，`D_tilde = D + I`。我们真正感兴趣的最终矩阵是`D_tilde⁻¹ = (D + I)⁻¹`：

![](img/Formula_B19153_06_025.jpg)

NumPy 有一个特定的函数，`numpy.identity(n)`，可以快速创建一个单位矩阵`I`，它的维度是`n`。在这个例子中，我们有四个维度：
...
我们可以通过实验验证这一点，计算`D_tilde⁻¹ A_tilde`和`A_tilde D_tilde⁻¹`。

![](img/Formula_B19153_06_032.jpg)

![](img/Formula_B19153_06_033.jpg)

的确，在第一个案例中，每一行的和等于 1。在第二个案例中，每一列的和等于 1。

矩阵乘法可以使用 `numpy.matmul()` 函数进行。更方便的是，Python 从版本 3.5 开始就有了自己的矩阵乘法运算符 `@`。让我们定义邻接矩阵`A`并使用这个运算符来计算矩阵乘法：
...
根据我们对这个架构的了解，我们预计当节点度数变化很大时，它的表现会更好。如果每个节点的邻居数量相同，这些架构是等效的：(`√deg(i)√deg(i) = deg(i)`):
