...
对于表格数据集，划分非常简单；它只需选择`n`样本（行）。然而，对于图数据集来说，这是一个问题——我们如何选择`n`节点而不破坏重要的连接？如果不小心，可能会导致一个孤立节点的集合，在这个集合中我们无法进行任何聚合。
...
为了解决这些问题，我们必须限制计算图的大小。在 GraphSAGE 中，作者提出了一种名为邻居采样的技术。我们不再将每个邻居都添加到计算图中，而是从中随机选择一个预定义数量的邻居。例如，在第一次跳跃中，我们只选择最多三个邻居，而在第二次跳跃中，我们选择最多五个邻居。因此，在这种情况下，计算图的节点数不能超过`3 × 5 = 15`。
...
我们将重点介绍均值聚合器，因为它是最容易理解的。首先，均值聚合器会取目标节点及其采样邻居的嵌入，将它们进行平均。然后，对这个结果应用一个带有权重矩阵的线性变换，`W`：

均值聚合器可以通过以下公式来总结，其中`σ`是一个非线性函数，比如 ReLU 或 tanh：
...
这里，`j`是在随机游走中与`u` 的邻居，`σ`是 Sigmoid 函数，`Pₙ(j)` 是`j` 的负采样分布，而`Q`是负样本的数量：
