...
局部启发式方法通过考虑两个节点的局部邻域来衡量它们之间的相似性。我们用`N(u)` 来表示节点`u`的邻居。以下是三种流行的局部启发式方法的示例：
...
+   **Katz 指数**计算两个节点之间每条可能路径的加权和。权重对应于折扣因子，`β ∈ [0,1]`（通常在 0.8 到 0.9 之间），用于惩罚较长的路径。根据这个定义，如果两个节点之间有许多（最好是短的）路径，它们更有可能连接。可以使用邻接矩阵的幂来计算任意长度的路径，`Aⁿ`，这就是为什么 Katz 指数定义如下：

    ![](img/Formula_B19153_10_008.jpg)
...
链接预测的矩阵分解受到了推荐系统[2]中先前工作的启发。通过这项技术，我们通过预测整个邻接矩阵`A_hat`间接预测链接。该操作是通过节点嵌入来完成的——相似的节点，`u`和`v`，应该有相似的嵌入，`zᵤ`和`zᵥ`。通过点积，我们可以将其写成如下形式：

+   如果这些节点相似，`zᵤᵀzᵥ`应该是最大的

+   如果这些节点是不同的，`zᵤᵀzᵥ`应该是最小的
...
这里，`Z`是节点嵌入矩阵。下图展示了矩阵分解如何工作的可视化解释：
...
矩阵分解的更高级变种包括拉普拉斯矩阵和`A`的幂。另一种解决方案是使用诸如`DeepWalk`和`Node2Vec`之类的模型。它们生成的节点嵌入可以配对以创建链接表示。根据 Qiu 等人[3]的研究，这些算法隐式地近似并分解复杂矩阵。例如，这是`DeepWalk`计算的矩阵：

![](img/Formula_B19153_10_026.jpg)

这里，`b`是负采样的参数。同样的情况也适用于类似的算法，如 LINE 和 PTE。尽管它们能够捕捉更复杂的关系，但它们仍然存在我们在*第三章*和*第四章*中看到的相同局限性：
...
+   **解码器**使用矩阵分解和 sigmoid 函数来逼近邻接矩阵，`A_hat`，输出概率：

![](img/Formula_B19153_10_031.jpg)

请注意，我们并不是要对节点或图进行分类。目标是为邻接矩阵的每个元素预测一个概率（在 0 和 1 之间），`A_hat`。这就是为什么 GAE 使用二元交叉熵损失（负对数似然）来训练，即两个邻接矩阵之间的元素：

![](img/Formula_B19153_10_033.jpg)

然而，邻接矩阵通常非常稀疏，这会使 GAE 偏向于预测零值。有两种简单的技术可以解决这个偏差。首先，我们可以在前面的损失函数中添加一个权重，以偏向`A[ij] = 1`。其次，我们可以在训练过程中采样更少的零值，从而使标签更加平衡。后一种技术是 Kipf 和 Welling 实施的。
...
+   **编码器**由两个共享第一层的 GCN 组成。目标是学习每个潜在正态分布的参数——均值，`μᵢ`（由`GCN[μ]`学习），和方差，`σᵢ²`（实际上，`log σ`由`GCN[σ]`学习）。

+   **解码器**从学习到的分布中采样嵌入，`zᵢ`，使用重参数化技巧[4]。然后，它使用潜在变量之间的相同内积来近似邻接矩阵，`A_hat = σ(ZᵀZ)`。
...
这里，`q(Z|X,A)`表示编码器，`p(Z)`是`Z`的先验分布。
...
1.  我们声明一个新类。在这个类中，我们需要三个 GCN 层——一个共享层，第二个层来近似均值，`μᵢ`，第三个层来近似方差值（实际上是对数标准差，`log σ`）：
...
1.  `train()`函数包括两个重要步骤。首先，嵌入矩阵，`Z`，是通过`model.encode()`计算的；这个名字可能有些不直观，但这个函数确实是从学习到的分布中采样嵌入。然后，计算 ELBO 损失，使用`model.recon_loss()`（二元交叉熵损失）和`model.kl_loss()`（KL 散度）。解码器会隐式调用来计算交叉熵损失：
...
在实际应用中，目标节点，`x`和`y`，必须共享一个唯一标签，以便将它们识别为目标节点。对于上下文节点，`i` 和`j`，如果它们与目标节点的距离相同——`d(i,x) = d(j,x)` 和`d(i,y) = d(j,y)` ，则必须共享相同的标签。我们称这种距离为双半径，记作 `(d(i,x), d(i,y))`。

可以考虑不同的解决方案，但 SEAL 的作者提出了 **双半径节点标签化**（**DRNL**）算法。其工作原理如下：

1.  首先，将标签 1 分配给`x`和`y`。

1.  将标签 1 分配给半径为`(1,1)`的节点。

1.  将标签 3 分配给半径为`(1,2)`或`(2,1)`的节点。

1.  将标签 4 分配给半径为`(1,3)`、`(3,1)` 等的节点。

DRNL 函数可以写作如下：

![](img/Formula_B19153_10_069.jpg)

这里，`d = d(i,x) + d(i, y)`、`d/2` 和`d%2`是分别将`d`除以 2 后得到的整数商和余数。最后，这些节点标签被一热编码。
...
1.  对于数据集中的每一对（源节点和目标节点），我们提取 k-hop 邻居（这里，`k = 2`）：
