...
感知器是一种简单的算法，它给定一个输入向量 `x`，包含 `m` 个值（`x₁`，`x₂`，...，`x[n]`），通常称为输入特征或简写为特征，输出为 `1`（是）或 `0`（否）。从数学上讲，我们定义一个函数：
...
Sigmoid 和 ReLU 通常被称为神经网络术语中的*激活函数*。在*Keras 中测试不同优化器*一节中，我们将看到这些逐步变化，典型的 Sigmoid 和 ReLU 函数，是发展学习算法的基本构建块，这些算法逐渐减少网络所犯的错误，一点点地适应。以下是使用激活函数 σ 与输入向量 (`x₁`, `x₂`, ..., `x[m]`)、权重向量 (`w₁`, `w₂`, *...*, `w[m]`)、偏置 `b` 和求和 Σ 的示例：
