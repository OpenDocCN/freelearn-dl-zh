...
*   如果是情景环境，则`J(θ)`可以是开始状态`V[π[θ]](s[1])`的值函数，也就是说，如果它从任何状态`s₁`开始，则其值函数将是该状态开始时的预期奖励总和 。 因此，
...
在**蒙特卡洛策略梯度**方法中，我们使用随机梯度上升方法更新参数，并按照策略梯度定理进行更新，并使用`vₜ`作为`Q[π[θ]](s[t], a[t])`的无偏样本。 在此，`vₜ`是从该时间步开始的累计奖励。
...
在此，`Aᵢ`是与状态操作对关联的优势。 优势是一个标量，它量化了操作最终的效果。 如果我们想在将来鼓励给定的行动，则`Aᵢ`会很高，而如果我们想阻止该行动，则`Aᵢ`会很低。 正优势使该状态的将来更有可能发生该行为，而负优势使该状态的将来不太可能发生该行为。
