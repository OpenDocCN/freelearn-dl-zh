...
因此，我们需要确保在预测给定时间的最优 Q 值时，贝尔曼方程中规定的条件得以保持。为此，我们可以将该模型的整体损失函数定义为最小化贝尔曼方程和实际预测中的误差。换句话说，在每次前向传播时，我们计算当前状态-动作质量值`Q (s, a ; θ)`与由贝尔曼方程在该时间（`Yₜ`）所表示的理想值之间的差距。由于由贝尔曼方程表示的理想预测正在被迭代更新，我们实际上是在使用一个移动目标变量（Y[t]）来计算我们模型的损失。这可以通过数学公式表示如下：

![](img/069e0250-a679-40a7-b27c-34686e709dc9.png)

因此，我们的网络将通过最小化一系列损失函数*Lt*来训练，在每个时间步骤中进行更新。这里，术语`yₜ`是我们在时间（`t`）进行预测时的目标标签，并在每个时间步骤中持续更新。另请注意，术语*ρ(s, a)*仅表示我们模型在序列 s 和执行的动作上的内部概率分布，也称为其行为分布。正如你所看到的，在优化给定时间（`t`）的损失函数时，前一时刻（*t-1*）的模型权重保持不变。虽然这里展示的实现使用相同的网络进行两个独立的前向传播，但后来的 Q 学习变体（Mihn 等，2015 年）使用两个独立的网络：一个用于预测满足贝尔曼方程的移动目标变量（称为目标网络），另一个用于计算给定时间的模型预测。现在，让我们看看反向传播是如何在深度 Q 学习中更新我们的模型权重的。
