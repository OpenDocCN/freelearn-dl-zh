...
第 1 层的初步训练是通过在实际输入`xᵢ`上进行训练得到的。第一步是优化编码器的*We(1)*层，以适应输出 X。上面的第二步是通过使用*We(1)*作为输入和输出，优化第二层的权重*We(2)*。一旦所有*We(i)*层（其中`i`=1, 2, ..., `n`为层数）都经过预训练，就可以通过将所有层连接在一起进行模型微调，正如前图中的第 3 步所示。该概念还可以应用于去噪训练多层网络，这被称为堆叠去噪自编码器。去噪自编码器中开发的代码可以轻松调整为开发**堆叠去噪自编码器**，它是堆叠自编码器的扩展。
