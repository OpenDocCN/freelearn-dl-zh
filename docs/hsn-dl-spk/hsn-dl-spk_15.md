...
在这里，`sₜ` 是时刻 `t` 的状态，`aₜ` 是代理采取的行动，`rₜ` 是时刻 `t` 的奖励，*s[t+1]* 是新的状态（时刻 *t+1*），![](img/645aec1f-4361-4b81-ac15-021c0fae6d8e.png) 是学习率 (![](img/653aec36-6dd8-4058-b906-55647168e2d6.png))，而 ![](img/d2d3c192-68f8-46c1-afc4-962f9294af48.png) 是折扣因子。最后一个参数决定了未来奖励的重要性。如果它为零，代理会变得目光短浅，因为它只会考虑当前的奖励。如果其值接近 1，代理则会努力实现长期的高奖励。如果折扣因子值为 1 或更高，则行动值可能会发散。
