...
应该注意的是，这种模型，其中数据被**标记**和**评分**，仅代表了现代机器学习能力的一半。另一类算法，比如生成了在佳士得拍卖会上出售的人造肖像的算法，不会从输入变量中计算得分或标签，而是**生成新数据**。与判别模型不同，输入变量通常是与现实世界值无关的数字向量，甚至经常是随机生成的。这种模型——被称为**生成模型**——可以从随机噪声中产生复杂的输出，例如文本、音乐或图像，并且是本书的主题。
