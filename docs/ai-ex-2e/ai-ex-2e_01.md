...
在这种情况下，状态`F`是状态 1，我们可以简化为写作`s₁`；B 是状态 2，我们可以简化为写作`s₂`；D 是`s₃`；C 是`s₄`。智能体正在计算所有这些可能的状态以做出决策。
...
当我们的智能体从一个状态转换到另一个状态时，它执行一次*转移*并获得奖励。例如，转移可以是从`F`到`B`，即从状态 1 到状态 2，或者从`s₁`到`s₂`。
