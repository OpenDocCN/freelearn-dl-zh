...
由于我们将使用 MI 来估计密度函数，因此 IIC 将采样限制为`(Z[i], Z_bar[i])`。 本质上，对于每个样本`xᵢ`，我们计算其潜在代码`P(Z[i]) = E(X[i])`。 然后，我们将`xᵢ`转换，并计算其潜在代码`P(Z_bar[i]) = E(X_bar[i])`。 联合分布计算如下：

![](img/B14853_13_051.png) (Equation 13.3.3)

其中`M`是批量大小。 由于我们对`xᵢ`和`x_bar[i]`使用相同的编码器`E`，因此联合分布应该对称。 我们通过执行以下命令来增强对称性：
