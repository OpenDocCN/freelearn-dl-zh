...
如前所述，VGG19 更高层次的特征用于作为内容表示。你可以把这些特征看作是滤波器的响应。为了表示风格，我们使用 Gram 矩阵 `G`（定义为向量 `v` 的矩阵 `v`ᐪ `v`），我们考虑 ![](img/B18331_03_002.png) 作为 VGG19 网络第 `l` 层中映射 `i` 和映射 `j` 的内积矩阵。可以证明，Gram 矩阵表示不同滤波器响应之间的相关性矩阵。
