...
WaveNet 的关键直觉是所谓的**扩张因果卷积**[5]（有时也称为**空洞卷积**），这意味着在应用卷积层的滤波器时，一些输入值被跳过。“Atrous”是法语表达式“à trous”的变形，意思是“带孔的”。因此，空洞卷积就是带孔的卷积。例如，在一维情况下，一个大小为 3、扩张为 1 的滤波器`w`将计算以下和：`w₀` `x₀` + `w₁` `x₂` + `w₂` `x₄`。
