...
为了简便起见，在本章中描述不同类型的 RNN 架构的所有方程中，我们省略了对偏置项的明确引用，而是将其并入矩阵中。考虑以下一个 n 维空间中的直线方程。这里，`w₁` 到 `w[n]` 是每个维度中直线的系数，偏置 `b` 是每个维度上的 y 截距：
...
换句话说，如果输入是序列[`c₁`, `c₂`, …, `c[n]`]，输出将是[`c₂`, `c₃`, …, `c[n+1]]`。我们将训练网络 50 个 epochs，在每个 10 个 epochs 的末尾，我们将生成一个以标准前缀开始的固定大小字符序列。在以下示例中，我们使用了前缀“爱丽丝”，这是我们小说中主人公的名字。
