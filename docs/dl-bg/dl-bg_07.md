...
这里，`o(z₁⁽¹⁾)`表示第一层中第一个神经元的激活函数的输出，在本例中将是一个 sigmoid 函数。

Sigmoid 激活函数表示为：

![](img/57a4a874-66bc-40f8-92bf-e2182fb49c42.png)

这个函数很有趣，因为它会将输入值压缩，并将其映射到 0 和 1 之间的值。它也是一个很好的用于梯度计算的函数，因为其导数是已知的，且易于计算：

![](img/8df4cea8-a438-4497-a673-8887fe5d4439.png)
...
现在我们已经计算了导数，我们可以使用传统的梯度缩放因子来更新权重，这个因子被称为**学习率**。我们计算新的`w*⁽²⁾`值，如下所示：
