...
该方程就是标准的二元交叉熵损失函数。损失是正确识别真实数据的期望值的负和，`D(x)`，以及正确识别合成数据的 1.0 减去期望值，`1 - D(g(z))`。对数不会改变局部最小值的位置。训练时，判别器会提供两小批次数据：

1.  `x`，来自采样数据的真实数据（即，`x ~ p_data`），标签为 1.0

1.  `x' = g(z)`，来自生成器的假数据，标签为 0.0

为了最小化损失函数，判别器的参数，`θ⁽ᵈ⁾`，将通过反向传播来更新，通过正确识别真实数据，`D(x)`，以及合成数据，`1 - D(g(z))`。正确识别真实数据等同于`D(x) -> 1.0`，而正确分类假数据则等同于`D(g(z) -> 0.0` 或者`1 - D(g(z) -> 1.0`。在这个方程中，`z`是生成器用来合成新信号的任意编码或噪声向量。两者共同作用于最小化损失函数。
...
有时，我们会通过伪装合成数据为真实数据（标签为 1.0）来试图欺骗判别器。通过相对于`θ⁽ᵈ⁾`的最大化，优化器向判别器参数发送梯度更新，使其将该合成数据视为真实数据。与此同时，通过相对于`θ⁽ᵍ⁾`的最小化，优化器将训练生成器的参数，教其如何欺骗判别器。然而，实际上，判别器在将合成数据分类为假数据时非常自信，因此不会更新其参数。此外，梯度更新很小，并且在传播到生成器层时已经大大减弱。因此，生成器未能收敛：
