...
如*图 1.3.3*所示，提出的 MLP 模型可用于 MNIST 数字分类。当单元或感知机被展示时，MLP 模型是一个完全连接的网络，如*图 1.3.4*所示。接下来还会展示如何根据输入计算感知机的输出，作为权重`wᵢ`和偏置`bₙ`（n 为第 n 个单元）的函数。相应的 Keras 实现请参见*Listing 1.3.2*。
